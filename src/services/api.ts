  ...
  getPoolsByTournament: async (tournamentId: string): Promise<Pool[]> => {
    ...
    if (result.success && Array.isArray(result.pools)) {
      return result.pools.map(p => ({
        id: String(p.poolId), // Force String
     ...
     if (result.success && Array.isArray(result.teams)) {
        return result.teams
          ...
          .map(t => ({
             ...
             poolId: t.poolId ? String(t.poolId) : "" // Force String
          }));
     }
    ...
    if (result.success && Array.isArray(result.matches)) {
        return result.matches
            ...
            .map(m => ({
                ...
                poolId: m.poolId ? String(m.poolId) : "", // Force String
                teamA: { id: String(m.teamA.id), name: m.teamA.name, score: m.teamA.score },
                teamB: { id: String(m.teamB.id), name: m.teamB.name, score: m.teamB.score },
                date: m.matchDate,
                time: m.matchTime,
                venue: m.venue,
                status: m.status,
  ...
  getStandings: async (): Promise<Standing[]> => {
    ...
    if (result.success && Array.isArray(result.standings)) {
        return result.standings.map(s => ({
            ...
            poolId: s.poolId ? String(s.poolId) : "", // Force String
